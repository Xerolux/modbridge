...
func (m *Manager) UpdateUser(id string, req *UpdateUserRequest) error {
	...
	if req.Password != nil && strings.TrimSpace(*req.Password) != "" {
		...
		// Administrator-issued credentials are temporary by definition.
		user.MustChangePassword = true
		passwordChanged = true
	}

	if req.MustChangePassword != nil && !passwordChanged {
...
// plaintext one-time password so the caller can log and display it; the account
// is marked must_change_password so the operator sets a personal password ASAP.
func (m *Manager) EnsureDefaultAdmin(username, password, createdBy string) (bool, error) {
	...
	// Bootstrap secrets are generated by the caller with crypto/rand. They are
	// hashed without applying the user-facing password policy.
